...
exports.createOffer = async (req, res) => {
  ...
  if (!req.file) {
    return res.status(200).json({ status: "failed", message: "Please select image" })
  }
  const offer = {
    ...
    offer_url: req.file.filename,
    user_id: req.user.user_id,
  };
  console.log(offer)
  try {
    const data = await Offer.create(offer);
    res.status(200).json({
      status: "success",
