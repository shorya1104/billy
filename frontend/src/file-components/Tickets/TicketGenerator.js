...
const Ticket_Register = () => {
    const ticketGenerate = () => toast('Ticket Generated Sucessfully');
    ...
    const handleNumInputsChange = (event) => {
        ...
        if (value > 0) {
            ...
            for (let i = 0; i < value; i++) {
                ...
                setTextboxes(prevState => [...prevState, { ticketid: offercode+generateUniqueNumber() }]);
            }
        }
    };
    ...
    const offerHandler = (e)=>{
       ...
       // return;
        if(e.target.value === ""){
            ...
            setOfferName(e.target.value.split("$")[0]);
            setOffercode(e.target.value.split("$")[1]);
        }
    }
                ...
                <Col className='d-flex justify-content-cente flex-column align-items-center rounded p-4 border-primary border'>
                    <form className='col-sm-12' autoComplete="off">
                        <div className="row mb-6">
                            ...
                            <select className="form-control fw-bolder fs-6 text-primary" name="offer" id="offer" onChange={offerHandler} onselec>
                                <option value="">Select Offer</option>
                               {offerList.length>0 && offerList.map((item)=>{
                                return(
                                <>
                                 <option value={item.offer_id+"$"+item.offer_code}>{item.offer_name}</option>
                                </>
                                )
                               })}
                        ...
                        <div  className='scrollHandler mb-3' style={{ height: (numInputs > 0) ? 'auto' : '0', overflowX: 'hidden', overflowY: 'scroll' }}>
